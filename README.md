...
双击运行后无需命令行也能调参：按 **Control-Option-Command-Comma（⌃⌥⌘,）** 打开设置面板，拖动滑块即时生效并自动记住，下次启动沿用。也可以在「终端」里用 `open -a Blackhole --args --size 8` 传入初始参数。

## 设置面板

无论用哪种方式运行，随时按 **⌃⌥⌘,** 打开设置面板，用滑块调整尺寸、漂移速度、屏幕数量和吸附增大。改动即时生效、自动保存，下次启动自动恢复。若你更习惯命令行，下面的参数依旧可用，且会作为初始值写入设置。

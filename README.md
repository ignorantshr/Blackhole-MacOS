...
- 捕获帧率联动：桌面抓取（ScreenCaptureKit 每帧整屏取样 + 色彩转换）是 CPU 占用的主项，与 GPU 渲染是两条独立路径。捕获基础帧率为 30fps、空闲时随渲染一起降到 15fps（此前固定 60fps 且不随空闲下降）。桌面内容与黑洞漂移都很慢，降捕获帧率肉眼无差，却直接对半砍掉这块 CPU 开销；多屏时每块显示器各一路捕获，收益随屏数叠加。捕获帧率通过 `updateConfiguration` 在运行时调整，无需重建流。

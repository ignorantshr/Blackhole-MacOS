...
- 逃逸光线查找表：黑洞附近像素原本每帧都要积分约 48 步 Schwarzschild 测地线。由于是中心力场，逃逸光线的偏折结果只是冲击参数的一元函数，故在启动时用 CPU 预计算成一维查找表（约 16KB，所有屏幕共享）。对确定不会穿过吸积盘的像素（占场内像素约 80%，由另一张按方位角索引的极小门控表保守判定），直接查表取回偏折，跳过整段积分；可能穿盘的像素与阴影核心仍走完整积分以保留吸积盘光照。表与分辨率、黑洞尺寸/位置、屏幕数量均无关，实测查表路径与完整积分的最终采样偏差在亚像素级（默认尺寸约 0.1px、最大尺寸约 0.3px），肉眼无法分辨。
